...
class customer //customer class
{
    ...
    long long int contact, carnumber, invoice_no, drivernumber;
};
class rent:public customer //inherited class from customer class
{
	public:
	int days,rentalfee,driverfee,totalfee; // variables
	char driver;
	void data()
	{
	int login();
    ...
    {   start0: 
        ...
        system("CLS");
        fstream Cars;
        //to open text files as per choice
    if(carmodel=="1") 
	...
	if(Cars.is_open())
	{
      ...
      if(carmodel!="1" && carmodel!="2" && carmodel!="3" && carmodel!="4" && carmodel!="5") //to check valid model or not
      cout<<"\n\n\t\t\tInvaild Car Model. Please try again!"<<endl;
    }
    while(carmodel!="1" && carmodel!="2" && carmodel!="3" && carmodel!="4" && carmodel!="5");
    cout<<"\n\n\t\t\t-------------------------------------------------------------------\n"<<endl;
    sleep(1);
    cout<<"\n\t\t\tPlease confirm:"<<endl; //to take confirmation for selecting cars
	cout<<"\n\t\t\tEnter 'Y' to confirm or 'N' to select again.\n\t\t\tTo exit Car Rental System enter any other alphabet."<<endl;
    ...
    { // maximum of 25 cars can be provided by company
    ...
    cin>>days;
    start2:
    cout<<"\n\t\t\tDriver Rent : Rs.2000 per day\n\t\t\tDo you need driver (Y/N) : "; // For driver requirements
    cin>>driver;
	...
	void calculate() //to calcualte rent
	{
		sleep(1);
		system ("CLS");
    ...
    system("PAUSE");    
    system ("CLS");    
    ifstream inf("Thanks.txt"); //To open thanks file
     char str[300];
     while(inf) 
     {
        inf.getline(str, 300);
...
int login() //login
{
   ...
   cout<<"\n\t\t\t\t\t\t     LOGIN \n";	
   cout<<"\t\t\t\t\t------------------------------\n\n";	
   cout<<"\t\t\t\t\tEnter Password: "; // accepting password from user
   ch = getch();
   while(ch != 13)
   ...
   if(pass == "pass") // if "pass" it grants access
   {
      cout << "\n\n\n\t\t\t\t\t\tAccess Granted! \n";
      system("PAUSE");
      system ("CLS");
   }
   else // else it asks to try again
   {
      cout << "\n\n\t\t\t\t\t\t\tAccess Aborted...\n\t\t\t\t\t\t\tPlease Try Again\n\n";
      system("PAUSE");
      system("CLS");
